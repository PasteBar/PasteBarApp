...
> = ({ children, outOutsideClick, isLargeModal, canClose, noFooter, positionTop }) => {
  const { height } = useWindowDimensions()
  const style = {
    maxHeight: noFooter ? height : height - 64,
  }
  return canClose ? (
    <Dialog.Content
      style={style}
      className={`${isLargeModal ? '-mt-5' : '-mt-80'} ${
        positionTop ? `!-mt-[100px]` : ''
    ...
    <Dialog.Root open={open} onOpenChange={handleClose}>
      <Portal.Portal ref={portalRef}>
        <ModalContext.Provider
          value={{ portalRef, isLargeModal, onOutsideClick, noFooter }}
        >
          <Overlay isNavVisible={isNavVisible} outOutsideClick={onOutsideClick}>
            <Content
              canClose={canClose}
              positionTop={positionTop}
              outOutsideClick={onOutsideClick}
              isLargeModal
              noFooter
            >
              {children}
...
Modal.Content = ({ children, className }) => {
  const { isLargeModal, onOutsideClick, noFooter } = React.useContext(ModalContext)

  const { height } = useWindowDimensions()
  const style = {
    maxHeight: noFooter ? height : height - 90 - 141,
  }
  return (
    <div
...
Modal.Header = ({
  ...
  return (
    <div
      data-tauri-drag-region
      className={`flex w-full items-center border-0 px-8 py-6 pb-2 ${className}`}
      onClick={e => {
        onOutsideClick && onOutsideClick()
        e.stopPropagation()
      }}
