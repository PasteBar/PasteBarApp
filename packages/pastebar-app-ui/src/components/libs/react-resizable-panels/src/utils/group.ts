...
const isDevelopment = import.meta.env.TAURI_DEBUG
    ...
    if (baseSize !== nextSize) {
      ...
      if (
        deltaApplied
          ...
          .localeCompare(Math.abs(deltaPixels).toPrecision(PRECISION), undefined, {
            numeric: true,
          }) >= 0
      ) {
        break
...
export function calculateDefaultLayout({
  ...
  for (let index = 0; index < panelsArray.length; index++) {
    ...
    if (defaultSize != null) {
      ...
      sizes[index] =
        units === 'pixels' ? (defaultSize / groupSizePixels) * 100 : defaultSize
    ...
    if (isDevelopment) {
      console.error(
        `Invalid panel group configuration; default panel sizes should total 100% but was ${(
          100 - remainingSize
        ).toFixed(1)}%. This can cause the cursor to become misaligned while dragging.`
      )
    }
  }
...
export function panelsMapToSortedArray(panels: Map<string, PanelData>): PanelData[] {
